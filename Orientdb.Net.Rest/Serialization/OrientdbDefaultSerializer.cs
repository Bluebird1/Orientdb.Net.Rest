...
{
    public class OrientdbDefaultSerializer : IOrientdbSerializer
    {
        ...
        public IEnumerable<Task> ReadStreamAsync<T>(Stream stream, TaskCompletionSource<T> tcs)
        {
            ...
            else
            {
                ...
                using (stream)
                {
                    ...
                    while (stream != null)
                    {
                        Task<int> read = Task<int>.Factory.FromAsync(stream.BeginRead, stream.EndRead, buffer, 0,
                            BUFFER_SIZE, null);
                        yield return read;
